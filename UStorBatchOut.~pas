...
type
  TFrmStorBatchOut = class(TFrmBase)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  FrmStorBatchOut: TFrmStorBatchOut;
...
procedure TFrmStorBatchOut.cxGrid1DBTableView1Column1GetDataText(
  Sender: TcxCustomGridTableItem; ARecordIndex: Integer;
  var AText: String);
 var
  recordcount:integer;
...
procedure TFrmStorBatchOut.FormShow(Sender: TObject);
var
 filterStr:String;
begin
  inherited;
...
procedure TFrmStorBatchOut.TblMainNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TFrmStorBatchOut.TblMainBeforePost(DataSet: TDataSet);
var
   ...
   begin
    ...
    with QryStorIn do
     begin
       ...
       // active:=true;
        open;
         Memo1.Lines.Add('已入库库存sql=['+sql.Text+']');
     ...
     Memo1.Lines.Add('1.已入库的库存数=['+inttostr(inNum)+']');
      ...
      with QryTemp do
           begin
              active:=false;
              Connection:=FConnection;
              sql.Text:='select sum(OUT_NUM) as num from kc_storbatch_out where BSN_NO ='+QuotedStr(bsNo) +' and MAT_CODE='+QuotedStr(StorInfo.MatNo);
             // active:=true;
              Execute;
        ...
        //3.再计算出本笔销售单里的库存数
         with QryTemp do
           begin
              active:=false;
              Connection:=FConnection;
              sql.Text:='select sum(OUT_NUM) as num from kc_storbatch_out where BSN_NO ='+QuotedStr(bsNo)+' and STOR_OUT_NO='+QuotedStr(strOutNo);
             // active:=true;
              Execute;
...
procedure TFrmStorBatchOut.TblMainAfterPost(DataSet: TDataSet);
 const qryNumSql='select sum(out_num) as num from kc_storbatch_out WHERE stor_out_no=''%s'' AND mat_code=''%s'' ';
...
procedure TFrmStorBatchOut.btnExportClick(Sender: TObject);
...
procedure TFrmStorBatchOut.TblMainAfterDelete(DataSet: TDataSet);
begin
  inherited;
//删除后更新
...
procedure TFrmStorBatchOut.cxTextSNKeyPress(Sender: TObject; var Key: Char);
var
  outNum,inNum,actNum,countNum:Integer;
  bsNo:String;
begin
  inherited;
    ...
    begin
           ...
           begin
              active:=false;
              Connection:=FConnection;
             sql.Text:='select mat_code,SUM(in_num) AS in_num,max(VALIDATE_DATE) as VALIDATE_DATE,max(degree) as degree   from KC_STOR_IN where BSN_NO ='+QuotedStr(bsNo) +' and MAT_CODE='+QuotedStr(StorInfo.MatNo) +' group by mat_code';
             // active:=true;
              Memo1.Lines.Add('sql=['+ sql.Text+']');
              Execute;
              if QryStorIn.RecordCount<=0 then
             ...
             begin
                active:=false;
                Connection:=FConnection;
                sql.Text:='select count(1) as count_num from kc_storbatch_out where BSN_NO ='+QuotedStr(bsNo) +' and MAT_CODE='+QuotedStr(StorInfo.MatNo)+' and STOR_OUT_NO='+QuotedStr(StorInfo.StorNo);
                 Memo1.Lines.Add('sql=['+ sql.Text+']');
                Execute;
                countNum:= QryTemp.fieldbyName('count_num').AsInteger;
                if countNum>=1 then
                begin
                   cxTextSN.Text:='';
                  // PlaySound(PChar('wrong.wav'),0, SND_ASYNC);
                   Application.MessageBox('本笔出库单记录中已存在该物料！','警告信息!',MB_OK);
                   Exit;
                end;
         ...
         begin
            active:=false;
            Connection:=FConnection;
            sql.Text:='select sum(OUT_NUM) as OUT_NUM from kc_storbatch_out where BSN_NO ='+QuotedStr(bsNo) +' and MAT_CODE='+QuotedStr(StorInfo.MatNo);
             Memo1.Lines.Add('sql=['+ sql.Text+']');
            Execute;
            outNum:=QryTemp.fieldbyName('OUT_NUM').AsInteger;
         end;
         actNum:=inNum- outNum;
         Memo1.Lines.Add('剩余库存=['+inttostr(actNum)+']');
          if actNum<=0 then
         ...
         try
           ...
           begin
              append;
              TblMain.FieldByName('BSN_NO').Value:=bsNo;
              //TblMain.FieldByName('STOR_IN_NO').Value:=QryStorIn.fieldByName('STOR_IN_NO').Value;
              TblMain.FieldByName('MAT_CODE').Value:=QryStorIn.fieldByName('MAT_CODE').Value;
              TblMain.FieldByName('DEGREE').Value:=QryStorIn.fieldByName('DEGREE').Value;
              TblMain.FieldByName('VALIDATE_DATE').Value:=QryStorIn.fieldByName('VALIDATE_DATE').Value;
              post;
           end;
           cxTextSN.Text:='';
...
procedure TFrmStorBatchOut.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TFrmStorBatchOut.btnImportClick(Sender: TObject);
var
